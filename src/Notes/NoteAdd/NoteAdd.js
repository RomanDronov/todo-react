...
export default class NoteAdd extends React.Component {
    ...
    saveNoteExternalTab(e) {
        ...
        }).then((res) => res.status).then((json) => {
            if (json === 201) console.log('ok');
            this.props.updateList();
            this.setState({ isExpandEdit: false });
    ...
    render() {
        ...
        if (this.state.isEditOpen) {
            editView = <div>
                ...
                <NoteEdit updateList={this.props.updateList}
                    closeEdit={this.closeEdit}
                    host={this.props.host}
                    color={this.state.color} />
                <button onClick={this.expandEditTab} > Expand editor </button> </div >;
        ...
        return (<div> {editView} {
            this.state.isExpandEdit && (<NoteEditTab saveNote={
                (e) => this.saveNoteExternalTab(e)
            }
                externalTabEditValue={
                    (e) => this.externalTabEditValue(e)
                } > < NoteEdit /> </NoteEditTab>
            )
        } </div>)
    }
}